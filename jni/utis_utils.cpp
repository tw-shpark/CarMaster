...
static const unsigned long crc32tab[256] = { 0x00000000, 0x77073096, 0xee0e612c,
...
JNIEXPORT jint JNICALL Java_com_teleworks_carmaster_CarmasterUtisFragment_shellCmd(
	...
	const char * j_cmdStr = env->GetStringUTFChars(cmdStr, 0);
	int cmdstatus;
	cmdstatus = system(j_cmdStr);
	LOGI("%s : status %d\n", j_cmdStr, cmdstatus);

	return cmdstatus;
}

JNIEXPORT jbyteArray JNICALL Java_com_teleworks_carmaster_CarmasterUtisFragment_genCrc16(
	...
	unsigned char * returnBuffer = new unsigned char[2];
	...
	unsigned short crc16 = crc16_ccitt((unsigned char*) cDataArray, jDataLen);

	returnBuffer[0] = (unsigned char) (crc16 >> 8);
	returnBuffer[1] = (unsigned char) crc16;

	LOGI("CRC16 [%02X %02X]\n", returnBuffer[0], returnBuffer[1]);

	jbyteArray result = env->NewByteArray(2);

	env->SetByteArrayRegion(result, 0, 2, (jbyte*) returnBuffer);
...
unsigned short crc16_ccitt(unsigned char *buf, int len) {
	...
	for (counter = 0; counter < len; counter++)
		crc = (crc << 8)
				^ crc16tab[((crc >> 8) ^ *(unsigned char *) buf++) & 0x00FF];
	return crc;
}
